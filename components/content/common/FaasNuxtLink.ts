...
import { preloadRouteComponents, onNuxtReady, navigateTo, useRouter, cancelIdleCallback, requestIdleCallback } from '#app'
...
/**
 * `<FaasNuxtLink>` is a drop-in replacement for standard <NuxtLink> to support versioned links. * 
 * @see https://nuxt.com/docs/api/components/nuxt-link
 */
/* @__NO_SIDE_EFFECTS__ */
export function defineNuxtLink (options: NuxtLinkOptions) {
    const componentName = options.componentName || 'FaasNuxtLink'
...
function applyTrailingSlashBehavior (to: string, trailingSlash: NuxtLinkOptions['trailingSlash']): string {
    ...
    if (hasProtocolDifferentFromHttp) {
        return useVersionedLink(to)
    }
    return normalizeFn(useVersionedLink(to), true)
}
